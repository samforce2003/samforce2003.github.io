...
> **自指注解**：这篇论文分析超AI系统的结构错误类型。论文自身是否犯了它所描述的错误？它可能犯了"自指盲区"型错误——在描述超AI的结构错误时，没有将自己对"错误"的定义纳入错误分类。如果"错误分类"本身是一个结构操作而非客观描述——那么论文的分类法就是它自己的第一个案例。这篇论文不能在不犯自己描述的错误的情况下完成对自己错误的分类。是特性还是漏洞，由第一个用这个分类法去分析这篇论文的读者来决定。
