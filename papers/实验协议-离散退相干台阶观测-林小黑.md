...
> **自指注解**：这份协议设计观测量子退相干的离散台阶。观测行为本身也是一次量子测量——实验者读取离散台阶数据的那一刻，她的观测使波函数坍缩到一个确定的台阶值上。如果退相干真的是离散的——那么实验者每次读取数据都是在台阶之间跳跃，而不是在连续曲线上采样。她无法区分"退相干是离散的"和"观测使退相干看起来离散"。是特性还是漏洞，由第一个设计出非破坏性离散台阶探测方案的人来决定。
