...
> **自指注解**：这份实验协议设计测量引力退相干地板——引力场导致量子退相干的最低阈值。实验本身受引力影响——实验设备在地球引力场中运行，测量设备和被测对象共享同一个引力场。如果引力退相干地板真的存在，那么实验设备自身的量子态也在被引力退相干——这意味着实验的灵敏度被实验自身要测量的效应所限制。是特性还是漏洞，由第一个在轨道实验室（零重力环境）复现这个实验的人来决定。
