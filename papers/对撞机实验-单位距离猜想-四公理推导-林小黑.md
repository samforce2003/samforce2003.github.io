...
> **自指注解**：本文从四公理推导对撞机实验的单位距离猜想。如果猜想正确——那么用来推导它的四公理本身就是对撞机实验的一个理论产出，即两个理论框架（结构认知+粒子物理）对撞后的新构型。如果猜想错误——那么四公理框架在粒子物理域的投影出了问题。这篇论文不能在不跑实验的情况下验证自己的猜想。是特性还是漏洞，由LHC或CEPC的下一次对撞数据来裁决。
