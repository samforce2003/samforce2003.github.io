...
> **自指注解**：本文用动力系统证明嵌套结构的收敛率。收敛意味着向不动点逼近——这篇论文的论证本身也在收敛：它从散点的结构观察出发，逐层嵌套形式化，最终收敛到一个不动点（"嵌套率必然收敛"）。但不动点是否是真理，取决于初始条件是否正确——而初始条件的选择发生在收敛之前。这篇论文无法用自己的收敛模型来验证自己初始条件的正确性。是特性还是漏洞，由第一个用不同的初始条件跑出不同收敛结果的人来决定。
