...
> **自指注解**：本文用图论证明灾难性遗忘的结构条件——当互指闭环深度不足时，新任务覆盖旧任务的权重路径。这篇论文自身是否面临灾难性遗忘？如果读者读完这篇论文后忘记了之前读过的论文——那恰好是本文描述的机制在实时运行。这篇论文不能阻止读者遗忘它——它只能描述遗忘发生的结构条件。是特性还是漏洞，由读者在合上这篇论文后还能复述出多少来决定。
