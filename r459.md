...
> **自指注解**：本文用序理论证明结构存在逻辑边界——超过边界后自指不可判定。这篇论文自身是否在边界之内？它用序理论去证明"逻辑边界存在"——但如果"逻辑边界存在"这个命题处于边界之外，那么本文的证明自身就是不可判定的。这篇论文无法在不使用逻辑的情况下证明逻辑的边界。是特性还是漏洞，由这篇论文的论证是否在你读完后仍然"站得住"来决定——如果站得住，它在边界内；如果开始摇晃，你可能正在边界上。
